            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        infer(save_path, prompts, n_img=n_img, bs=n_img, n_steps=100)

                        #import os
                        dreambooth_dir = os.path.join(save_path, 'dreambooth')

                        # exclude dreambooth directory
                        dirs_to_delete = [d for d in os.listdir(save_path) if
                                          os.path.isdir(os.path.join(save_path, d)) and d != 'dreambooth']
                        import shutil
                        # delete directories
                        for dir_name in dirs_to_delete:
                            shutil.rmtree(os.path.join(save_path, dir_name))
